...
```
makes compiler wait for a remote debugger to connect.

Note that command line of compiler run can be got from detailed gradle output during building your project. 
It's enough to run gradle command with `-i` flag.
...
Use `Edit Configurations` in dropdown menu with your run/debug configurations and add new `Remote JVM Debug`.
Choose in settings same port as you defined in `JAVA_OPTS` in `debug.sh` script.
